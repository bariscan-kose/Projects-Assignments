...
drop_features=["Month","Day","ProductName","Barcode","ContentId","Seller","VisualIntegrity","PSF","Keyword","KeywordRank","DiscountRate"]

sql_query[0].drop(drop_features, axis='columns', inplace=True)
corr_features=["BuyBox","SalesPrice","PurchaseQuantity","Hour","Visit"]

df=pd.read_excel(r'C:\Users\hp\Desktop\WorkBariscan\Correlation of Sales Metrics ML Project\SaldosData\Sky_Deneme.xlsx')
...
print("\nMean of Visit",sql_query[0]["Visit"].mean())
...
predictions_arr=[]
for pred in linear_est.predict(predict_input_fn):
  #predictions_arr.append(i["predictions"][0])
  for key, value in pred.items():
    predictions_arr.append(value[0])
