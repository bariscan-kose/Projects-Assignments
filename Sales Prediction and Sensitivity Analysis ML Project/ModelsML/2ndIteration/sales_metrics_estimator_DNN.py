...
table_name=["dbo.MainTable","dbo.CheckImageResults"]
...
#sql_query[0]=pd.read_excel(r'C:\Users\hp\Desktop\Work\Correlation of Sales Metrics ML Project\SaldosData\Sky_High.xlsx')

drop_features=["Month","Day","Hour","ProductName","Barcode","ContentId","Seller","VisualIntegrity","PSF","Keyword"]

sql_query[0].drop(drop_features, axis='columns', inplace=True)
corr_features=["BuyBox","SalesPrice","DiscountRate","PurchaseQuantity","Visit","KeywordRank"]
...
CATEGORICAL_COLUMNS = ["BuyBox"]
NUMERIC_COLUMNS = ["SalesPrice","DiscountRate","Visit","KeywordRank"]
feature_columns=[]
...
def dist_violin_plot(df_dfc, ID):
  ...
  parts=axes[0].violinplot([df_dfc[w] for w in ix], vert=False, showextrema=False, widths=0.7, positions=np.arange(len(ix)))
  ...
  # Add feature values.
  _add_feature_values(test_features.iloc[ID][sorted_ix], axes[0])

  # Add local contributions.
  axes[0].scatter(example,
  ...
  axes[0].plot([0,0], [1,1], label='eval set contributions\ndistributions', color=face_color, alpha=alpha, linewidth=10)
  legend = axes[0].legend(loc='lower right', shadow=True, fontsize='x-large', frameon=True)
  legend.get_frame().set_facecolor('white')

  # Format plot.
  axes[0].set_yticks(np.arange(example.shape[0]))
  axes[0].set_yticklabels(example.index)
  axes[0].grid(False, axis='y')
  axes[0].set_xlabel('Contribution to predicted value', size=14) 

dist_violin_plot(df_dfc, ID)
axes[0].set_title('Feature contributions for example {}\n pred: {:1.2f}; label: {}'.format(ID, preds[ID], labels[ID]))
...
axes[1].scatter(test_labels, test_predictions_Arr)
axes[1].set_xlabel("True Values [Sales]")
axes[1].set_ylabel("Predictions [Sales]") 
axes[1].plot(test_labels, test_labels, color="r")
